...
public class Searching {
	...
	private void proceedWithSearch() {
		...
		int index = search(numbers, target, 0, 9999);
	...
	private int search(int[] searchThis, int target, int startIndex, int endIndex) 
	{
		delay();
		int currentIndex = (int)((startIndex + endIndex) / 2);
		if (searchThis[currentIndex] == target)
		{
			return currentIndex;
		}
		else
		{
			...
			return search(searchThis, target, startIndex, endIndex);
		}
	}
